...
func (EvolutiveInjector) Start(elem string) {
	...
	for plugin := range oldPlugins {
		exec.Command("/bin/rm", shared2.DIR_PLUGINS+"/"+strings.TrimSpace(oldPlugins[plugin])).CombinedOutput()
	}
...
func changeSamePluginSeveralTimes(elem string) {
	...
	for {
		generatePlugin(elem, elem+"_v1")
		time.Sleep(shared2.INJECTION_TIME * time.Second)
	}
}
...
func generatePlugin(source, plugin string) {
	...
	pOut := shared2.DIR_PLUGINS + "/" + plugin
	pIn := shared2.DIR_PLUGINS_SOURCE + "/" + source + "/" + source + ".go"

	_, err := exec.Command(shared2.DIR_GO+"/go", "build", "-buildmode=plugin", "-o", pOut, pIn).CombinedOutput()
	if err != nil {

...
func clientX(client sendFile.SendFileServiceClient) {
	...
	for x := 0; x < SAMPLE_SIZE; x++ {
		...
		_, err := client.Upload(context.Background(), &sendFile.Request{Base64File: base64String})
		if err != nil {
			log.Fatal(";error", err, ";;\n")
		}
		//fmt.Printf("Fibo: %d => %d\n", n, response)
...
func main() {
	...
	defer conn.Close()
	client := sendFile.NewSendFileServiceClient(conn)

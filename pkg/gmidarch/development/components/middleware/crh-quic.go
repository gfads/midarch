...
// func (c CRHQUIC) getLocalTcpAddr() *net.TCPAddr {
...
func (c CRHQUIC) I_Process(id string, msg *messages.SAMessage, info *interface{}, reset *bool) {
	...
	if _, ok := crhInfo.Protocols[addr]; !ok || reflect.TypeOf(crhInfo.Protocols[addr]).Elem().Name() != "QUIC" { // no connection open yet
		...
		crhInfo.Protocols[addr] = &protocols.QUIC{}

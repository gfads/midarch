...
// func (c CRHHTTP2) getLocalTcpAddr() *net.TCPAddr {
...
func (c CRHHTTP2) I_Process(id string, msg *messages.SAMessage, info *interface{}, reset *bool) {
	...
	if _, ok := crhInfo.Protocols[addr]; !ok || reflect.TypeOf(crhInfo.Protocols[addr]).Elem().Name() != "HTTP2" { // no connection open yet
...
// func (c CRHHTTP2) send(sizeOfMsgSize []byte, msgToServer []byte, conn net.Conn) error {
// 	lib.PrintlnDebug("----------------------------------------->", shared.GetFunction(), "CRHHTTP2 Version Not adapted")
...
// func (c CRHHTTP2) read(conn net.Conn, size []byte) ([]byte, error) {
// 	lib.PrintlnDebug("----------------------------------------->", shared.GetFunction(), "CRHHTTP2 Version Not adapted")
